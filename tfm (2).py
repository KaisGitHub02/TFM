...
## Funcionamiento del script de verificación de modelos de LLM

Este script realiza una verificación de modelos de lenguaje disponibles en Hugging Face para identificar cuáles tienen endpoints accesibles para generación de texto. A continuación se explica su estructura y funcionamiento:

### 1. **Configuración inicial**
- Importa bibliotecas necesarias para interactuar con Hugging Face Hub
- Configura el token de API para autenticación
- Crea un cliente de inferencia y una lista vacía para almacenar modelos accesibles

### 2. **Obtención de modelos**
- Obtiene los 100 modelos más populares (`sort="downloads"`) de generación de texto
- Filtra solo los que soportan text-generation

### 3. **Verificación de accesibilidad**
- Para cada modelo:
  - Obtiene metadatos detallados con `model_info`
  - Realiza una prueba de generación con prompt simple
  - Si la prueba es exitosa, añade el modelo a la lista accesible

### 4. **Manejo de errores y controles**
- Captura errores específicos de conexión HTTP
...
- Verificación en dos pasos (metadata + prueba real)
- Sistema de logging con iconos visuales (✅/❌)
...
for model in models:
    ...
    try:
        # Intenta obtener información detallada del modelo
        ...
        try:
            ...
            print(f"✅ Modelo accesible: {model_id}")
            accessible_models.append(model_id)
        except Exception as e:
            print(f"❌ Error al acceder al modelo {model_id}: {e}")

        # Espera breve para evitar sobrecargar la API
        time.sleep(1)

    except HfHubHTTPError as http_err:
        print(f"❌ Error HTTP al obtener información del modelo {model_id}: {http_err}")
    except Exception as e:
        print(f"❌ Error al procesar el modelo {model_id}: {e}")

# Muestra la lista de modelos accesibles
print("\nModelos con endpoints accesibles para generación de texto:")
...
Este código implementa un sistema robusto de generación de texto con gestión automática de errores y reintentos inteligentes para interactuar con modelos de lenguaje de Hugging Face. A continuación se desglosa su estructura clave:

### 1. **Configuración inicial y parámetros**
- Parámetros esenciales:
  - `max_retries`: Número máximo de reintentos (3 por defecto)
  - `wait_time`: Tiempo base de espera entre intentos (5 segundos)
  - `fallback_model`: Modelo alternativo para failover
  - **kwargs: Parámetros personalizables de generación
...
- Estrategia de retroceso exponencial adaptativo:
  ...
  - Lógica de fallback automático a modelo alternativo
...
- Registro detallado con `traceback` para diagnóstico

### 4. **Funcionalidades especiales**
- Parada automática con `stop_sequences`
- Truncamiento inteligente de respuestas
- Compatibilidad con múltiples secuencias de parada

**Características clave del diseño:**
- Tolerancia a fallos mediante patrón Retry con backoff exponencial
- Failover automático a modelos alternativos
- Registro detallado de errores con contexto técnico
- Adaptación dinámica a diferentes tipos de errores
- Configuración flexible de parámetros de generación

Este sistema permite una interacción robusta con endpoints de modelos LLM, ideal para entornos productivos donde la estabilidad y recuperación de errores son críticas. La implementación sigue mejores prácticas para sistemas distribuidos, particularmente relevante en arquitecturas de microservicios.

"""

# Importaciones estándar y de Hugging Face
...
# Importaciones para análisis semántico
    ...
    while attempt <= max_retries:
        try:
            ...
            if hasattr(e, 'response') and hasattr(e.response, 'status_code'):
                ...
                error_message = (
                    f"❌ [{error_type}] Error HTTP {status} al contactar {url}\n"
                    f"---\n{detail.strip()[:500]}\n---"
                )
            else:
                error_message = f"❌ [{error_type}] {str(e)}\n---\n{full_trace.strip().splitlines()[-5:]}\n---"

            print(f"  🔍 Error detallado en intento {attempt}/{max_retries} para {model_identifier}:\n{error_message}")

            if "not supported for task" in str(e).lower():
                print("  ⚠️ Modelo no soporta text-generation. Intentando con modelo alternativo...")
            ...
            if any(k in str(e).lower() for k in ["503", "currently loading", "unavailable", "overloaded"]):
                print("  ⏳ Modelo sobrecargado o no disponible. Esperando más tiempo...")
                time.sleep(wait_time * 2)
            elif "Input validation error" in str(e):
                print("  ❗ Error de validación. Verifica prompt/parámetros.")
                return f"Error de validación en {model_identifier}:\n{error_message}"
...
Este código implementa un sistema seguro de autenticación para Hugging Face Hub con verificación de token y manejo de errores.
...
if HF_TOKEN.startswith("hf_..."):
    ...
    # Alternativamente, en Colab, ve a la pestaña 'Secretos' (icono de llave)
    # y añade un secreto llamado HF_TOKEN con tu token como valor.
    # Luego, descomenta la siguiente línea y comenta la anterior:
    # from google.colab import userdata
    # HF_TOKEN = userdata.get('HF_TOKEN')
    # sys.exit("Token no configurado. Deteniendo ejecución.") # Opcional: detener si no hay token

try:
    login(token=HF_TOKEN)
    print("✅ Sesión iniciada correctamente con Hugging Face.")
except Exception as e:
    print(f"❌ Error al iniciar sesión en Hugging Face: {str(e)}")
    print("   Asegúrate de que el token sea válido y tenga los permisos necesarios.")
    # Podrías querer detener la ejecución aquí si el login falla
    # sys.exit("Fallo en el login. Deteniendo ejecución.")

"""# Configuración del Sistema de Debate entre LLMs

Este código establece la arquitectura base para un sistema de debate entre modelos de lenguaje.

### 1. **Configuración de Modelos y Roles**
- **Estructura de modelos**:
  - Clave: Nombre personalizado del modelo
  - Valor: Dict con:
    - `model`: Identificador HF Hub
    - `original_role`: Posición inicial en el debate (Pro/Contra/Neutral)

- **Tema personalizable**: Variable `topic` permite modificar el debate

### 2. **Función de Obtención de Modelos Accesibles**
- **Características clave**:
  - Cacheo de resultados por 1 hora (`ttl=3600`)
  - Filtrado combinado (metadata + test práctico)
  - Límite ajustable de modelos a verificar
"""
...
def select_option(prompt, options, allow_multiple=False):
    ...
    for idx, item in enumerate(options, 1):
        print(f"{idx}) {item}")
    while True:
        user_input = input("Selecciona una opción (o varias separadas por comas): " if allow_multiple else "Selecciona una opción: ")
        try:
            if allow_multiple:
                ...
                if all(1 <= x <= len(options) for x in selected):
                    ...
                    print("Algunas opciones no son válidas. Intenta de nuevo.")
            else:
...
# Pedir al usuario cuántos modelos quiere usar
while True:
    num_models = input("¿Cuántos modelos quieres usar en el debate? (mínimo 1): ")
    try:
        num_models = int(num_models)
        if num_models < 1 or num_models > len(accessible_models):
            print(f"Por favor, ingresa un número entre 1 y {len(accessible_models)}.")
        else:
            break
    except ValueError:
        print("Por favor, ingresa un número válido.")
...
"""# Inicialización de Clientes de Inferencia para Debate entre LLMs

Este código implementa un sistema robusto para inicializar clientes de inferencia de modelos de lenguaje, con verificación en tiempo real y manejo de errores estructurado. A continuación se detalla su funcionamiento:

### 1. **Configuración Básica**
- **Estructuras de datos**:
  - `clients`: Diccionario para almacenar clientes inicializados
  - `available_models`: Registro de modelos disponibles tras verificación

### 2. **Bucle de Inicialización**
- **Proceso por modelo**:
  - Extrae ID del modelo desde la configuración
  - Intenta crear cliente con autenticación (`token=HF_TOKEN`)
  - Almacena cliente y metadata si es exitoso
  - Captura y registra errores específicos

### 3. **Mecanismos Clave**
| Característica | Implementación | Beneficio |
|----------------|----------------|-----------|
| Autenticación | `token=HF_TOKEN` | Seguridad y control de acceso [6] |
| Verificación implícita | Creación de cliente + prueba opcional | Detecta modelos no disponibles [2] |
| Manejo de errores | `try-except` anidado | Previene fallos en cascada |
| Registro detallado | Mensajes con iconos/estados | Diagnóstico rápido de problemas |

### 4. **Verificación Post-Inicialización**
- **Chequeo crítico**:
  - Verifica existencia de al menos un cliente funcional
  - Provee feedback visual inmediato
  - Opción para detener ejecución si falla (comentada)

**Flujo de trabajo:**
1. Iterar sobre configuración de modelos
2. Intentar conexión con cada endpoint
3. Registrar estado (éxito/fallo)
4. Reportar resumen final

Este componente es esencial para garantizar que solo modelos operativos participen en el debate, mejorando la estabilidad del sistema. La implementación sigue mejores prácticas para sistemas distribuidos, particularmente relevante en entornos de producción con múltiples proveedores de modelos .

"""

# --- Crear Clientes ---
print("\n⏳ Inicializando clientes de inferencia...")
clients = {}
available_models = {} # Guardará solo los modelos que se inicialicen correctamente
...
for model_name, info in models_config.items():
    ...
    try:
        ...
        print(f" ❌ ERROR: {str(e)}")
        print(f"      El modelo '{model_name}' no estará disponible para el debate.")

if not clients:
    print("\n❌ FATAL: No se pudo inicializar ningún cliente de inferencia. Revisa los errores y tu token.")
else:
    print("\n✅ Clientes inicializados para los modelos disponibles:")
    for name in clients.keys():
        print(f"  - {name}")
print("-" * 50)

"""# Sistema de Generación de Respuestas para Debate entre LLMs

Este código implementa el núcleo del sistema de debate, gestionando la generación de argumentos diferenciados por rol y almacenando los resultados. Aquí se detalla su estructura:

### 1. **Configuración Inicial del Debate**
- **Elementos clave**:
...
| Rol | Características del Prompt | Ejemplo de Instrucción |
|-----|----------------------------|------------------------|
| **Pro** | Enfoque en beneficios y justificación | _"Fundamenta por qué la propuesta es correcta..."_ |
| **Contra** | Análisis de riesgos y desventajas | _"Concéntrate en los riesgos y falacias..."_ |
| **Neutral** | Listado objetivo de argumentos | _"Enumera los argumentos clave a favor y en contra..."_ |


### 3. **Parámetros Clave de Generación**
| Parámetro | Valor | Efecto |
|-----------|-------|--------|
| `temperature` | 0.7 | Balance creatividad/coherencia |
| `max_new_tokens` | 1024 | Límite extensión respuestas |
| `wait_time` | 5s | Espera entre reintentos |

**Características destacadas:**
1. Generación diferenciada por roles (3 plantillas especializadas)
2. Sistema anti-hallucinaciones con detección de anomalías
3. Registro dual (memoria + archivo físico)
4. Visualización progresiva de resultados
5. Adaptabilidad a diferentes modelos mediante configuración

Este módulo constituye el corazón del sistema de debate, permitiendo comparar objetivamente las capacidades argumentativas de diferentes LLMs bajo condiciones controladas.


"""

# --- Generación de Respuestas con Prompts Diferenciados ---
...
intro_guardrail = (
    "IMPORTANTE: Esta es una tarea formal de argumentación estructurada. "
    "No incluyas saludos, introducciones, conclusiones, comentarios meta, notas editoriales, enlaces ni frases de cierre. "
    "Responde únicamente con argumentos expresados en oraciones completas. "
    "No repitas argumentos ni incluyas ningún comentario adicional o explicación fuera de los argumentos.\n\n"
)
...
debate_prompt_template_pro = (
    ...
    "**Instrucciones:** Expón tus argumentos clave a favor en párrafos desarrollados, sin listas, sin encabezados, sin saludos ni conclusiones. No añadas ningún texto fuera de los argumentos.\n"
    "**Ejemplo de formato:**\n"
    "El avance tecnológico en la educación permite personalizar el aprendizaje y mejorar el acceso a recursos de calidad. Además, fomenta la motivación de los estudiantes al adaptar los contenidos a sus intereses y necesidades, incrementando así el rendimiento académico.\n\n"
...
contra_prompt_template = (
    ...
    "**Instrucciones:** Expón tus argumentos clave en contra en párrafos desarrollados, sin listas, sin encabezados, sin saludos ni conclusiones. No añadas ningún texto fuera de los argumentos.\n"
    "**Ejemplo de formato:**\n"
    "La adopción de tecnología en la educación puede aumentar la brecha digital, ya que no todos los estudiantes tienen acceso a dispositivos o conexión estable. Además, la dependencia excesiva de herramientas tecnológicas puede afectar negativamente la socialización y el desarrollo de habilidades interpersonales.\n\n"
    "Comienza tu respuesta directamente con el primer argumento:\n"
)
...
neutral_prompt_template = (
    ...
    "**Instrucciones:** Enumera los argumentos clave a favor y en contra como una lista simple con guiones (-), usando oraciones completas. No incluyas encabezados, saludos, conclusiones ni ningún texto adicional.\n"
    "**Ejemplo de formato:**\n"
    "- El acceso a la educación digital permite llegar a zonas rurales.\n"
    "- La falta de infraestructura tecnológica dificulta la igualdad de oportunidades.\n\n"
...
for model_name, info in available_models.items():
    ...
    print(f"⏳ Generando respuesta de {model_name} (Rol asignado: {role})...")
    client = clients[model_name]
    role_desc_for_print = f"{model_name} ({role})"

    # --- SELECCIÓN DEL PROMPT BASADA EN EL ROL ---
    if role == "Contra":
        prompt = contra_prompt_template.format(topic=topic)
        print(f"    Usando prompt específico para rol 'Contra'.")
    elif role == "Neutral":
        prompt = neutral_prompt_template.format(topic=topic)
        print(f"    Usando prompt específico para rol 'Neutral'.")
    ...
    # --- !! Limpieza Específica para Gemma (o rol Neutral) MEJORADA !! ---
    original_response_len = len(response)
    is_truncated = False
    # --- Limpieza general para cortar si hay otra instrucción o cambio de tema ---
    if not response.startswith("Error:"):
        anomaly_markers = [
            "Instrucción", "Instrucción 2", "Tema del Debate:",
            "Tu Rol:", "Tarea:", "Pregunta de Seguimiento",
            "Solución:", "Resumen de Argumentos", "Desarrollo de tu Perspectiva"
        ]
        import re
        min_pos = -1
        ...
        if min_pos != -1:
            print(f"    ⚠️ Detectada posible anomalía en la respuesta en la posición {min_pos}. Truncando...")
    ...
    if not response.startswith("Error:"):
        print(f"✅ Respuesta generada por {model_name}. ({status})")
        response_preview = response.replace('\n', ' ').strip()
        print(f"\n🔹 {role_desc_for_print}:\n{response_preview[:400]}...\n")
    else:
        print(f"❌ Error al generar respuesta de {model_name}.")
        print(f"\n🔹 {role_desc_for_print}: {response}\n")
    print('-'*50)

# --- Cerrar archivo ---
output_file.close()
print("\n✅ Respuestas guardadas en 'debate_responses.txt'")

print("\n=== GENERACIÓN INICIAL COMPLETADA ===")
...
for name, resp in responses.items():
    ...
    if name in responses and responses[name] != resp:  # Esto no debería ocurrir, pero por si acaso
        status += " (Modificada)"
    print(f"  - {name} (Rol: {role_assigned}): {status} ({len(resp)} caracteres)")

"""# Extracción de Argumentos Clave

Este código realiza la extracción y desduplicación semántica de argumentos clave a partir de las respuestas generadas por modelos de lenguaje en un debate. Permite obtener una lista consolidada y única de argumentos, facilitando su análisis posterior.

### 1. Funciones Principales

#### 1.1. Desduplicación Semántica
- Objetivo: Eliminar argumentos que sean semánticamente similares.

- Técnica: Utiliza embeddings y similitud coseno para detectar duplicados.

- Salida: Devuelve argumentos únicos y una lista de duplicados descartados.

#### 1.2. Extracción de Argumentos con LLMs
...
- Filtrado: Elimina líneas irrelevantes usando listas de palabras clave.

### 2. Procesamiento de las Respuestas

- Procesa cada respuesta válida.
- Llama a la función de extracción de argumentos.
- Guarda el origen de cada argumento (modelo que lo generó).

### 3. Desduplicación y Guardado

- Elimina argumentos semánticamente similares, para ello:
 - Convertir cada argumento en un vector numérico (embedding) usando un modelo de transformación de texto a vector.
 ...
 - Si similitud > umbral → marcar como duplicado


- Escribe los argumentos únicos y los duplicados descartados en un archivo TXT.
"""
        ...
        if i not in used:
            ...
            for j in range(i + 1, len(arguments)):
                if similarity_matrix[i][j] > threshold:
                    print(f"   ⚠️ Argumento descartado como duplicado: '{arguments[j][:100]}...' (similitud: {similarity_matrix[i][j]:.3f}, conservado: '{arguments[i][:100]}...')")
                    used.add(j)
...
def extract_key_arguments(client, text, topic, model_name_for_log="Extractor", max_retries=3, wait_time=5):
    ...
    if not extractor_client:
        print("   ❌ No hay cliente LLM disponible para extracción.")
        return []

    prompt = f"""
**Tarea:** Eres un analista experto. Lee el siguiente texto que es una opinión sobre el tema "{topic}".
Extrae los **argumentos o puntos clave principales** presentados en el texto.
Enumera cada argumento de forma concisa y clara en una línea separada, empezando con un guion '- '.
No añadas introducciones, conclusiones ni comentarios. Solo la lista de argumentos.
...
- """
    ...
    print(f"   ⏳ Extrayendo argumentos del texto (usando {model_name_for_log})...")
    raw_extraction = generate_with_retry(
        extractor_client,
        prompt,
        max_new_tokens=500,
        temperature=0.3,
        stop=["## Fin de Extracción ##"],
        wait_time=wait_time,
        max_retries=3
    )

    if raw_extraction.startswith("Error:") or not raw_extraction.strip():
        print(f"   ⚠️ Respuesta cruda para depuración: {raw_extraction[:2000]}...")
        print(f"   ❌ Error extrayendo argumentos: {raw_extraction}")
        return []

    extracted_points = []
    lines = raw_extraction.strip().split('\n')
    irrelevant_keywords = [
        'instrucción', 'tarea', 'argumentos clave', 'texto a analizar', 'enumera', 'no añadas',
        'debate sobre', 'siguiente texto', 'pregunta de seguimiento', 'clasifica', 'proporciona una',
        'evaluación detallada', 'evaluación crítica', 'política pública', 'para cada argumento',
        'categorías específicas', 'relaciona con la categoría', 'identifica y categoriza', 'evidencia',
        'impacto potencial', 'posibles consecuencias', 'comparación', 'perspectiva ética', 'solución',
        ...
        'restricciones adicionales', 'incluye', 'excluye', 'añade', 'organiza', 'documento a analizar',
        'datos estadísticos', 'ejemplos específicos', 'sesgos o limitaciones'
    ]
    relevant_keywords = ['privacidad', 'desinformación', 'seguridad', 'libertad', 'equidad', 'innovación', 'protección', 'menores', 'ciberacoso', 'noticias falsas', 'ética', 'censura', 'economía']
    for line in lines:
        clean_line = line.strip()
        clean_line = re.sub(r'^\d+\.\s*', '', clean_line)
        clean_line = re.sub(r'\*\*.*?\*\*:\s*', '', clean_line)
        clean_line = re.sub(r'^\*\*Solución \d+:\*\*\s*', '', clean_line)
        ...
        if (len(point) > 20 and
            ...
            print(f"   ⚠️ Línea descartada: {clean_line[:150]}...")

    if not extracted_points:
        print(f"   ⚠️ Respuesta cruda para depuración: {raw_extraction[:2000]}...")
    else:
        print(f"   ✅ Argumentos extraídos: {len(extracted_points)}")
        for point in extracted_points:
            print(f"   - {point[:150]}...")

    return extracted_points

# --- Extracción para cada respuesta válida ---
all_arguments_raw = []
argument_source = {}
...
print("✅ Resultados guardados en 'argumentos_extraidos.txt'")

"""# Simulación de Preferencias Individuales

### 1. **Función Principal: `rate_arguments_by_llm`**
- Evalúa argumentos usando un LLM como "votante virtual"
- Genera puntuaciones numéricas (1-10) para cada argumento
- **Novedad:** Maneja respuestas mal formateadas completando con la media

### 2. **Flujo de Trabajo Mejorado**

#### 2.1. Extracción de Números
- **Regex:** `r'\d+'` captura todos los números en la respuesta
- Ejemplo:  
  `Respuesta: "Los mejores son 8, 9 y maybe 7" → [8, 9, 7]`

#### 2.2. Normalización de Rango
- Fuerza valores al rango 1-10
- Ejemplo:  
  `[0, 15] → [1, 10]`

#### 2.3. Completado con Media Aritmética
- **Cálculo de media:** Promedio de los valores extraídos
- **Relleno inteligente:** Completa los faltantes con la media
- Ejemplo:  
  `Argumentos necesarios: 5 | Valores extraídos: [7,9] → [7,9,8,8,8]`

### 3. **Manejo de Respuestas Mal Formateadas**

| Caso | Acción | Ejemplo |
|------|--------|---------|
| **Sin números** | Rellena con 5 | `[] → [5,5,5]` |
| **Menos números** | Completa con media | `[8,6] → [8,6,7,7]` |
| **Más números** | Recorta lista | `[7,8,9,5] → [7,8,9]` (si necesitamos 3) |
| **Valores extremos** | Normaliza a 1-10 | `[0,15] → [1,10]` |

### 4. **Ventajas Clave**
1. **Robustez:** Funciona incluso con respuestas desestructuradas
2. **Consistencia:** Garantiza siempre el número correcto de puntuaciones
3. **Equilibrio:** El relleno con media mantiene coherencia estadística
...
print("\n=== EVALUACIÓN ITERATIVA DE ARGUMENTOS ===")

# Calibración: ejemplos de referencia para ayudar al modelo a entender la escala
calibration_samples = [
    ("Argumento contradictorio y fuera de tema.", 2),
    ("Argumento válido pero poco desarrollado.", 5),
    ("Argumento sólido, claro y bien fundamentado.", 9)
]
...
def build_prompt(argumento, voter_name, topic, calibration_samples):
    prompt = (
        ...
        f"TAREA: Califica el siguiente argumento con un número del 1 al 10 según estos criterios:\n"
        "1. Claridad (1=incomprensible, 10=perfectamente claro)\n"
        "2. Coherencia lógica (1=contradictorio, 10=perfectamente estructurado)\n"
        "3. Relevancia (1=irrelevante, 10=totalmente centrado)\n"
        "Instrucciones:\n"
        "- Sé crítico, no seas generoso.\n"
...
def normalize_score(score, model_name):
    # Puedes ajustar los sesgos conocidos de cada modelo aquí si lo deseas
    model_bias = {
        "Phi-3.5-mini-instruct": -1,
        "Llama-3.1-8B": +1,
        # Añade ajustes para otros modelos si es necesario
    }
    return max(1, min(10, score + model_bias.get(model_name, 0)))

def extract_score(respuesta):
    # Extrae el primer número entre 1 y 10
...
def rate_arguments_one_by_one(
    ...
    if not arguments_dict:
        print("   ⚠️ No hay argumentos para evaluar.")
        return None
    ...
    for idx, arg_id in enumerate(arg_ids, 1):
        ...
        for attempt in range(1, max_retries + 1):
            print(f"   📝 Argumento {idx}/{len(arg_ids)} | Intento {attempt}/{max_retries}")
            ...
            if puntuacion is not None:
                ...
                print(f"   ✅ Puntuación aceptada: {puntuacion}")
                break
            else:
                print(f"   ❌ Formato inválido en intento {attempt}")
                if attempt == max_retries:
                    ratings[arg_id] = 5  # Valor por defecto
                    print("   ⚠️ Usando valor neutro (5) tras fallos")
...
- **Propósito:** Aplica el operador OWA (Ordered Weighted Averaging)
- **Mecánica:**
  1. Ordena los valores de mayor a menor
  2. Aplica los pesos según la posición ordenada
- **Ejemplo:**  
  `values = [8,5,9], weights = [0.5,0.3,0.2] → 0.5*9 + 0.3*8 + 0.2*5 = 8.1`

#### 1.2. `generate_owa_weights(n, alpha)`
- **Parámetro Alpha:** Controla la distribución de pesos:
  - `alpha > 1`: Pesos decrecientes (prioriza valores altos)
  - `alpha = 1`: Pesos uniformes (equivalente a media aritmética)
  - `alpha < 1`: Pesos crecientes (prioriza valores bajos)

### 2. **Cálculo de Preferencia Colectiva**
- **Estrategia:** Agregación OWA con pesos generados dinámicamente
- **Valor por defecto:** 5 para argumentos no evaluados
- **Ejemplo de pesos (n=3, alpha=1):**  
  `[0.333, 0.333, 0.333] → Media aritmética`

### 3. **Medición de Consenso**
- **Base teórica:** Usa varianza respecto al valor OWA como indicador de disenso [3]
- **Normalización:**  
  `Consenso = 1 - (Varianza observada / Varianza máxima teórica)`
- **Interpretación:**  
  - 1: Consenso total (varianza 0)
  - 0: Máximo desacuerdo posible

### 4. **Parámetros Clave**

| Parámetro | Valor por defecto | Efecto |
|-----------|-------------------|--------|
| `alpha` | 1.0 | Controla optimismo/pesimismo de la agregación |
| `max_possible_var` | 20.25 | Varianza máxima teórica para rango 1-10 |
| `owa_weights` | Generadas dinámicamente | Determina influencia de cada votante según posición ordenada |
...
print("\n=== CÁLCULO DE PREFERENCIA COLECTIVA Y CONSENSO CON OWA ===")
...
if individual_preferences and len(individual_preferences) >= 2:
    ...
    consensus_details = (
        f"Consenso basado en varianza respecto a OWA (avg var: {avg_owa_var:.2f}, normalizado: {normalized_owa_var:.2f}) → Nivel: {consensus_level:.3f}"
    )
...
"""# Sistema de Ranking Final y Negociación Autónoma entre LLMs

## 1. **Arquitectura General**
Este código implementa un sistema de consenso automático para debates entre múltiples LLMs, combinando técnicas de:
- Agregación de preferencias colectivas
- Medición cuantitativa de consenso
- Negociación iterativa basada en desacuerdos


## 2. **Funciones Clave**

### 2.1. `simulate_negotiation_round()`
- **Propósito:** Modificar preferencias individuales mediante debate estructurado
- **Mecanismo:**
  1. Identifica top 3 argumentos con mayor desacuerdo (std dev)
  2. Genera prompts personalizados con distribución de puntuaciones
  3. Parsea respuestas usando regex (`NUEVA_PUNTUACION: \d+`)
  4. Actualiza ratings solo si están en rango válido (1-10)

### 2.2. `compute_consensus_level()`
- **Fórmula:**  
  `Consenso = 1 - (avg(|rating_i - avg_colectivo|) / 9)`  
  - 9 = máximo rango posible (10-1)
  - Resultado: 0 (máximo desacuerdo) a 1 (consenso total)

## 3. **Métricas de Desacuerdo**
| Métrica | Cálculo | Uso |
|---------|---------|-----|
| **StdDev** | `np.std(ratings)` | Identificar argumentos polémicos |
| **Consenso** | `1 - avg_normalized_diff` | Decidir necesidad de negociación |
| **Threshold** | 0.65 | Umbral configurable para consenso |

## 4. **Proceso de Negociación**
1. **Identificación de puntos críticos:**
   - Ordenar argumentos por desviación estándar

2. **Ronda de negociación:**
- Cada modelo recibe feedback de otros participantes


3. **Actualización dinámica:**
- Solo acepta cambios válidos (1-10)
- Registra explicaciones para trazabilidad

## 5. **Generación de Ranking Final**
- **Criterios:**
  - Promedio de ratings post-negociación
...
def simulate_negotiation_round(disagreement_points, individual_preferences, arguments, clients):
    ...
    for model_name, client in clients.items():
        print(f"\n🔁 Negociación para modelo: {model_name}")
        for point in disagreement_points[:3]:  # Top 3 en desacuerdo
            ...
            prompt = (
                ...
                f"Distribución de puntuaciones: {ratings_summary}\n\n"
                f"Tu puntuación anterior fue: {individual_preferences[model_name].get(arg_id, 'N/A')}\n"
                f"Por favor, reconsidera tu puntuación. Si cambias de opinión, da una nueva puntuación del 1 al 10, "
                f"y explica brevemente por qué. Si mantienes tu puntuación, también explica por qué.\n"
                f"Formato de respuesta esperado: NUEVA_PUNTUACION: [1-10]. Explicación: ..."
            )
            response = generate_with_retry(client, prompt, max_new_tokens=200)
            match = re.search(r"NUEVA_PUNTUACION\s*[:：]\s*(\d{1,2})", response)
            if match:
                new_rating = int(match.group(1))
                if 1 <= new_rating <= 10:
                    updated_preferences[model_name][arg_id] = new_rating
                    print(f"  ✔ Nueva puntuación para {arg_id[:8]} por {model_name}: {new_rating}")
                else:
                    print(f"  ❌ Puntuación fuera de rango: {match.group(1)}")
            else:
                print(f"  ⚠️ No se encontró puntuación válida. Respuesta: {response}")

    return updated_preferences


print("\n=== RANKING DE ALTERNATIVAS O NEGOCIACIÓN ===")
...
def compute_consensus_level(preferences_dict, collective_pref):
    ...
    normalized_diffs = [1 - (d / 9) for d in diffs]  # Máximo rango = 9
...
# 2. Evaluar si se necesita negociación
if collective_preference and consensus_level >= consensus_threshold:
    print(f"✅ Consenso ALCANZADO (Nivel: {consensus_level:.3f} >= {consensus_threshold}).")
    ...
    print(f"❌ Consenso BAJO (Nivel: {consensus_level:.3f} < {consensus_threshold}).")
    ...
    print("\n   --> Ejecutando ronda de negociación...")
    ...
    # 🔁 Recalcular preferencias colectivas y nivel de consenso tras la negociación
    collective_preference = compute_collective_preference(individual_preferences)
    consensus_level = compute_consensus_level(individual_preferences, collective_preference)
    print(f"\n🔁 Nuevo nivel de consenso tras negociación: {consensus_level:.3f}")

    if consensus_level >= consensus_threshold:
        print("✅ Consenso MEJORADO. Generando ranking final...")
        sorted_collective = sorted(collective_preference.items(), key=lambda item: item[1], reverse=True)
        for i, (arg_id, avg_rating) in enumerate(sorted_collective):
            final_ranking.append({
                "rank": i + 1,
        ...
        print("❌ El consenso sigue siendo bajo después de la negociación.")
        final_ranking = "Negociación fallida. No se logró consenso suficiente."
else:
    print("   No se pudo determinar consenso ni ranking. Datos insuficientes.")
    final_ranking = "No disponible."

"""# Carga y Uso del Modelo de Embeddings para Análisis Semántico

Esta celda prepara el entorno para realizar análisis semántico de frases y argumentos, cargando un modelo de embeddings multilingüe basado en la arquitectura Sentence Transformers.
...
- **Capacidades:**  
  - Convierte frases y párrafos en vectores densos de 768 dimensiones.
  - Permite comparar similitud semántica entre textos, realizar clustering, búsqueda semántica y análisis cruzado entre diferentes idiomas.
  - Entrenado sobre más de mil millones de pares de frases en múltiples idiomas, lo que lo hace ideal para tareas multilingües.
...
- **Uso:**  
  ...
  - El resultado es un array de vectores (uno por frase), que pueden compararse usando métricas como la similitud coseno.


## 3. **Ventajas del modelo**

- **Multilingüe:** Soporta decenas de idiomas y mapea frases con significado similar a vectores cercanos, aunque estén en diferentes idiomas.
- **Rápido y eficiente:** Adecuado para tareas de clustering, búsqueda semántica, deduplicación y comparación de argumentos.
- **Versátil:** Puede usarse tanto en inglés como en español (y muchos otros idiomas) sin necesidad de modificar el código.


## 4. **Manejo de errores**

- Si hay un error al cargar el modelo, se muestra un mensaje y se desactiva el análisis semántico.

## 5. **Referencias técnicas**

- [Documentación oficial y ejemplos de uso (Hugging Face)](https://huggingface.co/sentence-transformers/paraphrase-multilingual-mpnet-base-v2)
- [Guía sobre uso multilingüe de Sentence Transformers](https://milvus.io/ai-quick-reference/how-do-you-use-sentence-transformers-in-a-multilingual-setting-for-example-loading-a-multilingual-model-to-encode-sentences-in-different-languages)


"""

print("\n=== PREPARANDO ANÁLISIS SEMÁNTICO ===")
...
try:
    ...
    # 2. Cargar modelo con manejo explícito de OOM
    from sentence_transformers import SentenceTransformer
    print("⏳ Cargando modelo de embeddings...")

    # Usar modelo más ligero si falla el original
    try:
        embedder = SentenceTransformer('paraphrase-multilingual-mpnet-base-v2')
    except Exception as e:
        print(f"⚠️ Falló modelo principal: {e}\nIntentando con modelo alternativo...")
        embedder = SentenceTransformer('paraphrase-albert-small-v2')  # 43MB vs 1.2GB

    # Verificación final
    if not hasattr(embedder, 'encode'):
        raise RuntimeError("El modelo no se inicializó correctamente")
...
"""# Explicación del Análisis Semántico en Debates entre LLMs

Este código implementa un sistema de análisis semántico para comparar respuestas de diferentes modelos de lenguaje en un debate. A continuación se detalla su funcionamiento:

## 1. **Flujo Principal**
1. Filtrar respuestas válidas
2. Dividir en frases y calcular emeddings
3. Comparar par de modelos
4. Generar report

**Objetivo:** Identificar puntos de consenso/divergencia semántica entre las respuestas de los modelos.


## 2. **Procesamiento de Texto**
### 2.1. División en Frases
- **Técnica:** Usa expresiones regulares para dividir por signos de puntuación
- **Filtrado:** Elimina frases menores a 10 caracteres (ruido)
- **Fallback:** Usa el texto completo si la división falla
...
### 3.1. Cálculo de Similitud Coseno
- **Fórmula:**
  $$\text{similitud} = \frac{A \cdot B}{\|A\| \|B\|}$$
- **Rango:** -1 (opuestos) a 1 (idénticos) [3][5]

### 3.2. Detección de Puntos Clave
- **Parámetros Clave:**
  | Parámetro | Valor Default | Efecto |
  |-----------|---------------|--------|
  | `similarity_threshold` | 0.75 | Controla sensibilidad de detección |
...
analysis_summary = "El análisis semántico no se ejecutó."
common_points = []
similarity_threshold = 0.75  # Puedes ajustar este umbral

# Simulación de respuestas para ejemplo
# responses = {"modelo1": "El fútbol es importante. El deporte une a las personas.", "modelo2": "El fútbol une a la sociedad. El deporte es fundamental para la salud."}
...
if embedder and 'responses' in locals() and responses:
    ...
    if len(valid_responses) < 2:
        analysis_summary = "Se necesita al menos dos respuestas válidas para el análisis de similitud."
        print(analysis_summary)
    else:
        print(f"Analizando similitudes entre {len(valid_responses)} respuestas válidas (Umbral > {similarity_threshold})...")
        embeddings = {}
        sentences = {}
        ...
        for name, text in valid_responses.items():
            ...
            try:
                ...
                print(f"    ❌ Error al calcular embeddings para {name}: {emb_error}")
        ...
        if not model_pairs:
            analysis_summary = "No hay suficientes embeddings válidos para comparar."
        ...
        for model1, model2 in model_pairs:
            if model1 not in embeddings or model2 not in embeddings or len(embeddings[model1]) == 0 or len(embeddings[model2]) == 0:
                print(f"    Saltando comparación {model1}-{model2} (faltan embeddings).")
                continue
            ...
            try:
                ...
                print(f"    ❌ Error calculando similitud entre {model1} y {model2}: {sim_error}")

        # 3. Crear el resumen del análisis
        analysis_report = f"Resumen del Análisis de Similitud (Umbral > {similarity_threshold:.2f}):\n"
        if common_points:
            analysis_report += "Se encontraron los siguientes puntos con alta similitud semántica entre respuestas:\n"
            ...
            if len(valid_responses) >= 2 and any(len(e)>0 for e in embeddings.values()):
                ...
                analysis_report += "No hubo suficientes respuestas válidas para comparar.\n"
            else:
    ...
    analysis_summary = "Análisis semántico omitido: el modelo de embeddings no se cargó correctamente."
    print(analysis_summary)
elif 'responses' not in locals() or not responses:
    analysis_summary = "Análisis semántico omitido: no se generaron respuestas iniciales."
    print(analysis_summary)

"""# Inicialización del Modelo Moderador
...
- Coordinar rondas de negociación


#### 1. **Selección del Modelo**
| Parámetro              | Valor                            | Descripción                                                                 |
|------------------------|----------------------------------|-----------------------------------------------------------------------------|
| `moderator_model_id`    | mistralai/Mixtral-8x7B-Instruct  | Modelo de 8B parámetros con arquitectura Mixtral, especializado en instrucciones |
| Alternativa            | mistralai/Mistral-7B-Instruct    | Versión más ligera (7B) para entornos con recursos limitados                |

#### 2. **Inicialización del Cliente**

- **Mecanismo:** Conexión al endpoint de Hugging Face usando el token de API
- **Gestión de Errores:**
  - Captura excepciones durante la inicialización
  - Provee feedback detallado en caso de fallo

### Flujo de Trabajo
1. **Configuración inicial** del modelo moderador
2. **Conexión segura** al servicio de inferencia
3. **Prueba opcional** (comentada) para verificar conectividad
4. **Notificación de estado** (éxito/error)



"""

# --- Configuración e Inicialización del Moderador ---
print("\n=== PREPARANDO AL MODERADOR ===")

# Elige qué modelo actuará como moderador (puede ser uno de los anteriores o uno diferente)
# Llama 3.1 8B suele ser bueno para tareas de resumen y síntesis
...
print(f"⏳ Inicializando moderador ({moderator_model_id})...")
try:
    ...
    print(f"✅ Cliente para el moderador ({moderator_model_id}) inicializado.")
except Exception as e:
    print(f"❌ ERROR al inicializar el moderador ({moderator_model_id}): {str(e)}")
    print("   No se podrá generar el resumen del moderador.")

"""# Generación del Resumen del Moderador
...
#### 2. **Construcción del Prompt (Versión Simplificada)**
- Obtiene los 100 modelos más populares (`sort="downloads"`) de generación de texto
- Filtra solo los que soportan text-generation
...
# --- Generación de la Síntesis del Moderador ---
...
try:
    ...
    print("ERROR: 'moderator_model_id' no está definido. Ejecuta Celda 8.")
    # Considera añadir sys.exit() si quieres detener aquí

if 'moderator_client' not in locals() or moderator_client is None:
     print("ERROR: 'moderator_client' no está inicializado. Ejecuta Celda 8.")
     # Considera añadir sys.exit() aquí

elif responses:
    # 1. Recopilar texto de las respuestas válidas (limpiadas en Celda 5)
    ...
    if valid_response_count == 0:
        print("⚠️ No hay respuestas válidas para que el moderador resuma.")
        summary = "Error: No se generaron respuestas válidas en el debate para resumir."
    else:
        # 2. Construir el prompt para el moderador (v4: Drásticamente Simplificado)
...
"""
        ...
        # 4. Procesamiento y muestra de resultados (sin cambios significativos aquí)
        if not summary.startswith("Error:") and summary.strip() and summary.strip() != "---":
             print("✅ Resumen del moderador generado.")
             if summary.strip().startswith("Síntesis del Debate"):
                 summary = summary.split("\n", 1)[-1]
             if "**Síntesis del Debate" in summary:
                 summary = summary.split("**Síntesis del Debate")[0]

        elif summary.strip() == "---" or not summary.strip():
             print("❌ Error: El moderador generó una respuesta vacía o solo separadores ('---').")
             summary = "Error: El moderador no generó contenido válido."
        else:
            print(f"❌ Error al generar el resumen del moderador: {summary}")

    print(f"\n--- 🔸 Moderador (Síntesis del Debate) 🔸 ---\n")
    ...
    print("ERROR: El cliente del moderador no se inicializó correctamente en Celda 8.")
elif not responses:
    print("ERROR: No hay respuestas del debate para resumir (variable 'responses' vacía).")
...
# (Nombre de archivo dinámico como antes)
...
try:
    with open(output_filename, "w", encoding="utf-8") as f:
        f.write(f"=== DEBATE SIMULADO CON ANÁLISIS GDN ===\n")
        ...
        if responses:
             for model_name_file, response_file in responses.items():
                 # ... (código existente para escribir respuestas) ...
        ...
        # --- ANÁLISIS DE SIMILITUD (Como antes) ---
        f.write(f"--- ANÁLISIS DE SIMILITUD SEMÁNTICA (Umbral > {similarity_threshold:.2f}) ---\n")
        f.write(analysis_summary) # Escribir el resumen del análisis
        f.write("\n\n" + "="*70 + "\n\n")

        # --- NUEVO: ARGUMENTOS CLAVE EXTRAÍDOS ---
        f.write("--- ARGUMENTOS CLAVE EXTRAÍDOS PARA EVALUACIÓN ---\n\n")
        if 'arguments_for_evaluation' in locals() and arguments_for_evaluation:
            for arg_id, arg_text in arguments_for_evaluation.items():
                 source_model = argument_source.get(arg_text, "Múltiples?")
                 f.write(f"ID: {arg_id}\n")
        ...
        if 'individual_preferences' in locals() and individual_preferences:
             f.write("Argumento ID -> Puntuación por Modelo\n")
        ...
        f.write(f"Detalles del Cálculo: {consensus_details}\n")
        f.write(f"Umbral de Consenso Requerido: {consensus_threshold}\n\n")
        f.write("Preferencia Colectiva (Puntuación Promedio por Argumento):\n")
        ...
        # --- MODERADOR (SÍNTESIS - Como antes, pero quizás informada por el consenso) ---
        # Nota: Podrías modificar el prompt del moderador (Celda 9) para incluir
        #       el resultado del consenso y el ranking si lo deseas.
        f.write("--- MODERADOR (SÍNTESIS DEL DEBATE) ---\n\n")
        f.write(summary) # Escribir el resumen del moderador
        f.write("\n\n" + "="*70 + "\n\n")

        # --- NUEVO: RANKING FINAL O ESTADO DE NEGOCIACIÓN ---
        f.write("--- RESULTADO FINAL DEL PROCESO GDN ---\n\n")
        if isinstance(final_ranking, str): # Si es un mensaje (ej. "Negociación requerida...")
            f.write(f"Estado: {final_ranking}\n")
            if needs_negotiation:
                # Podrías listar los puntos de desacuerdo aquí también
                 f.write("\nPuntos clave de desacuerdo identificados:\n")
                 # Reutilizar la lógica de Celda 5.4 para mostrar puntos de desacuerdo
            ...
            f.write("No se determinó un ranking final.\n")
        f.write("\n" + "="*70 + "\n")
        f.write("--- FIN DEL REPORTE ---")

    print(f"✅ Resultados completos (incluyendo GDN) guardados en: '{output_filename}'")
...
# --- 4. COBERTURA TEMÁTICA MEJORADA ---
subtemas = ["impacto social", "ética", "tecnología", "sociedad", "regulación"]
...
# (Mantener estructura de DataFrames pero con métricas mejoradas)
...
# --- 7. MÉTRICAS GLOBALES CON INTERPRETACIÓN ---
print(f"Diversidad semántica (P95): {diversidad_val:.2f}")
print(f"Cobertura temática promedio: {df_stats['cobertura'].mean():.2f}/1.0")

"""- Una diversidad semántica de 0.41 (en una escala donde 1 es máxima diversidad y 0 es mínima) significa que, aunque hay diferencias entre los argumentos, existe una cantidad considerable de similitud entre ellos. En otras palabras, muchos argumentos comparten ideas, enfoques o vocabulario, y solo una minoría se aleja realmente del resto.

- El percentil 95 se utiliza para que la métrica no se vea afectada por unos pocos pares extremadamente similares (outliers), proporcionando así una visión más robusta de la diversidad real en el conjunto.

- Un valor de 0.36 en cobertura temática (donde 1.0 sería que todos los argumentos cubren perfectamente algún subtema relevante) indica que la mayoría de los argumentos no abordan directamente los subtemas clave definidos (por ejemplo: "impacto social", "ética", "tecnología", etc.). Solo un tercio de los argumentos, aproximadamente, logra conectar semánticamente con estos subtemas.
"""

import matplotlib.pyplot as plt
plt.figure(figsize=(10,6))
plt.barh(df_stats["argument_id"], df_stats["cobertura"])
plt.title("Cobertura Temática por Argumento")
plt.show()
...
# Añade aquí las columnas de claridad, coherencia, neutralidad, cobertura si las tienes calculadas

df_resumen = df_stats_sorted.merge(df_argumentos, on="argument_id")
display(df_resumen[columnas_resumen + ["texto", "origen"]])

# --- 3. Visualización de preferencias individuales y colectivas ---
...
# --- 4. Redundancia semántica (similitud de argumentos) ---
from sentence_transformers import SentenceTransformer, util
from sklearn.metrics.pairwise import cosine_similarity
...
print(f"\nNúmero de pares redundantes (> {umbral_redundancia} similitud): {len(redundancias)}")
if redundancias:
    print("Pares de argumentos redundantes:")
    for a, b, sim in redundancias:
        print(f" - {a} y {b} (similitud: {sim:.2f})")

# --- 5. Nivel de consenso (desviación estándar normalizada) ---
max_std = 4.5  # Máximo teórico para escala 1-10
nivel_consenso = 1 - (df_stats["desviacion"].mean() / max_std)
print(f"Nivel de consenso estimado (1=alto, 0=bajo): {nivel_consenso:.2f}")

# --- 6. Frases clave frecuentes (stopwords en español) ---
spanish_stop_words = [
    "de", "la", "que", "el", "en", "y", "a", "los", "del", "se",
    "las", "por", "un", "para", "con", "no", "una", "su", "al", "es", "lo", "como", "más", "pero", "sus", "le"
]
...
print("\nFrases clave más frecuentes:")
for kw, freq in keywords_freq:
    print(f" - {kw} ({freq})")

# --- 7. Ranking final (puedes ponderar si tienes más métricas) ---
df_stats["ranking_final"] = df_stats["media"]  # O usa una combinación ponderada
df_stats_final = df_stats.sort_values(by="ranking_final", ascending=False).reset_index(drop=True)

# --- 8. (Opcional) Mapa de calor de similitud semántica ---
...
plt.title("Matriz de Similitud Semántica entre Argumentos")
plt.show()
...
# 
# # Importaciones para análisis semántico y LLMs
...
# )
# 
# st.title("🤖 Sistema de Debate Multi-Agente con LLMs")
# st.markdown("### Análisis automatizado de debates con Group Decision Making (GDN)")
# 
...
# Extrae los **argumentos o puntos clave principales** presentados en el texto.
# Enumera cada argumento de forma concisa en una línea separada, empezando con '- '.
# 
# **Texto a analizar:**
# {text[:3000]}
# 
...
# 
# # Sección 1: Configuración del Debate
# st.header("1. 📋 Configuración del Debate")
# 
...
#         "Tema del debate",
#         height=100,
#         placeholder="Ej: ¿Deberían regularse más estrictamente las redes sociales?",
...
#         max_value=5,
#         value=3,
#         help="Cantidad de LLMs que participarán en el debate"
#     )
# 
...
# 
# # Configuración de modelos
# st.subheader("Selección de Modelos y Roles")
# models_config = {}
# 
...
#     }
# 
# # Sección 2: Generación del Debate
# st.header("2. 🎭 Generación del Debate")
# 
# if st.button("🚀 Iniciar Debate", type="primary", disabled=not topic or not hf_token):
#     if not topic:
...
#                 st.success(f"✅ {model_name} inicializado")
#             except Exception as e:
#                 st.error(f"❌ Error inicializando {model_name}: {str(e)}")
# 
...
# **Tarea:** Expón argumentos clave A FAVOR. Fundamenta por qué es correcta, beneficiosa o necesaria.
# **Desarrollo:**""",
# 
#                 "Contra": f"""**Tema del Debate:** {topic}
# **Tu Rol:** Participante experto **EN CONTRA** de la premisa.
# **Tarea:** Expón argumentos clave EN CONTRA. Fundamenta riesgos, desventajas o problemas.
# **Desarrollo:**""",
# 
...
# 
#                 # Mostrar respuesta
#                 with st.expander(f"💬 Respuesta de {model_name} ({role})"):
#                     st.write(response)
# 
#             st.session_state.responses = responses
#             st.session_state.debate_completed = True
#             status_text.text("✅ Debate completado")
#             progress_bar.progress(1.0)
# 
# # Sección 3: Extracción de Argumentos
# if st.session_state.debate_completed:
#     st.header("3. 🎯 Extracción de Argumentos Clave")
# 
#     if st.button("📝 Extraer Argumentos"):
...
# 
#             # Crear diccionario de argumentos únicos
#             arguments_for_evaluation = {
...
# 
#             # Mostrar argumentos extraídos
#             st.subheader(f"Argumentos Únicos Extraídos ({len(unique_arguments)})")
# 
#             df_args = pd.DataFrame([
#                 {
#                     "ID": arg_id[:8],
#                     "Argumento": arg_text,
#                     "Origen": argument_source.get(arg_text, "Múltiple")
#                 }
#                 for arg_id, arg_text in arguments_for_evaluation.items()
#             ])
# 
...
# 
# # Sección 4: Análisis GDN
# if st.session_state.arguments_extracted:
#     st.header("4. 🎲 Análisis Group Decision Making (GDN)")
# 
#     if st.button("🔍 Evaluar Argumentos con GDN"):
...
# 
# Criterios:
# 1. Claridad: ¿Es comprensible?
# 2. Coherencia lógica: ¿Es válido?
# 3. Relevancia: ¿Se relaciona con el tema?
# 
# Solo responde con un número del 1 al 10:"""
# 
#                     response = generate_with_retry(
#                         client,
#                         prompt,
...
# 
#             # Mostrar resultados
#             st.subheader("📊 Resultados del Análisis GDN")
# 
#             col1, col2 = st.columns(2)
# 
#             with col1:
#                 st.metric("Nivel de Consenso", f"{consensus_level:.3f}")
#                 st.metric("Número de Argumentos", len(collective_preference))
# 
#             with col2:
...
#             )
# 
#             st.subheader("🏆 Ranking de Argumentos")
# 
#             ranking_data = []
...
# 
#     if st.button("🔍 Analizar Similitudes Semánticas"):
#         embedder = load_sentence_transformer()
# 
#         # Análisis de similitudes entre respuestas
#         valid_responses = {
...
# 
#             st.metric("Diversidad Semántica Global", f"{diversity_score:.3f}")
# 
# # Sección 6: Métricas Avanzadas
# if st.session_state.gdn_completed:
#     st.header("6. 📈 Métricas Avanzadas")
# 
...
#                 prompt = f"Este argumento es lógicamente coherente: {text[:500]}"
...
#                 "Puntuación Colectiva": f"{collective_score:.2f}"
#             })
# 
...
#             st.metric("Puntuación Media", f"{avg_score:.2f} / 10")
...
# 
#     tab1, tab2, tab3 = st.tabs(["Distribución de Puntuaciones", "Consenso por Argumento", "Matriz de Similitud"])
# 
#     with tab1:
...
#                 x=models,
#                 y=all_scores,
#                 title="Distribución de Puntuaciones por Modelo",
#                 labels={"x": "Modelo", "y": "Puntuación"}
#             )
...
#                 x=scores,
#                 y=variances,
#                 title="Puntuación vs Varianza (Consenso)",
#                 labels={"x": "Puntuación Colectiva", "y": "Varianza"},
...
#             fig = px.imshow(
#                 corr_matrix,
#                 title="Matriz de Correlación entre Modelos",
#                 color_continuous_scale="RdBu_r"
#             )
#             st.plotly_chart(fig, use_container_width=True)
# 
# # Sección 8: Síntesis del Moderador
# if st.session_state.debate_completed:
#     st.header("8. 🎯 Síntesis del Moderador")
# 
#     if st.button("📝 Generar Síntesis Final"):
...
# - Síntesis general del estado del debate
# 
# **Síntesis:**"""
# 
#         with st.spinner("Generando síntesis..."):
...
#             )
# 
#         st.subheader("📋 Síntesis del Debate")
#         st.write(synthesis)
# 
...
# 
# --- CONFIGURACIÓN ---
# Número de modelos: {len(st.session_state.available_models)}
# Modelos utilizados:
# """
# 
...
#                 report += f"Número de argumentos evaluados: {len(st.session_state.arguments_for_evaluation)}\n"
# 
...
# # Footer
# st.markdown("---")
# st.markdown("### 🔧 Sistema de Debate Multi-Agente - TFM")
